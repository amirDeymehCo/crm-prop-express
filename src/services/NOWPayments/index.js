...
const NOW_BASE_URL = process.env.NOWPAYMENTS_BASE_URL;
const NOW_API_KEY = process.env.NOWPAYMENTS_API_KEY;
const NOW_IPN_SECRET = process.env.NOWPAYMENTS_IPN_SECRET;

// axios instance

console.log(NOW_BASE_URL);
console.log(NOW_API_KEY);
console.log(NOW_IPN_SECRET);

const client = axios.create({
  baseURL: "https://tel2.myprop.trade/nowpayments-proxy/api/v1/invoice",
  headers: {
...
async function createDepositUSDInvoice({
  user,
  amountUsd,
  callback_url = null,
}) {
  ...
  const body = {
    ...
    ipn_callback_url:
      callback_url ||
      `${process.env.API_BASE_URL}/api/v1/user/wallet/deposit/nowpayment/ipn`,
    success_url: `${process.env.FRONT_BASE_URL}/wallet/deposit-success`,
    cancel_url: `${process.env.FRONT_BASE_URL}/wallet/deposit-cancel`,
  };

  const { data } = await client.post("/", body); // endpoint رسمی invoice :contentReference[oaicite:3]{index=3}
...
async function handleIpnCallback(rawBody, signatureHeader) {
  ...
  if (["finished", "confirmed"].includes(payment_status)) {
    ...
    await sequelize.transaction(async (t) => {
      ...
      const newBalance = parseFloat(wallet.balance) + parseFloat(price_amount);
