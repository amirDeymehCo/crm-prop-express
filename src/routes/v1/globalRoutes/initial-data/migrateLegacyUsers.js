...
/**
 * ایمپورت دسته‌ای کاربران و چالش‌های آن‌ها به صورت ایمن و بهینه
 * @param {Array} legacyUsers - آرایه‌ای از کاربران مطابق الگوی نمونه
 * @param {String} sourceSystem - نام سیستم مبدا مثل 'legacy_crm'
 */
async function migrateUsersBatch(legacyUsers, sourceSystem = "legacy_crm") {
  ...
  try {
    ...
    const userPayloads = legacyUsers.map((u) => ({
      ...
      password: u.password_hash, // رمزهای hash شده قبلی مستقیما بدون اعمال Hook درج می‌شوند
    ...
    if (challengesToInsert.length > 0) {
      importedChallenges = await UserChallenge.bulkCreate(challengesToInsert, {
        transaction,
        updateOnDuplicate: [
          "current_phase_id",
          "current_phase_index",
 ...
 */
async function rebuildReferralRelations(
  legacyUsers,
